BUILD_CGO_ENABLED  := 0
SERVICE_NAME       := object-storage-proxy
REBASE_URL         := "github.com/dimaskiddo/object-storage-proxy"
...
rebase:
	rm -rf .git
	find . -type f -iname "*.go*" -exec sed -i '' -e "s%github.com/dimaskiddo/object-storage-proxy%$(REBASE_URL)%g" {} \;
	git init
